...
Sub RefreshButton_Click()
  ...
  Const filter = SetupSheet.ListObjects("WkTable").Range.Value
...
Public Sub WeekComboBox_Change()
  ...
  Const filter As Variant = Range("FCWeeks").Value
  Range("TD").AutoFilter Field:=1, Criteria1:=filter
  Range("CD").AutoFilter Field:=1, Criteria1:=filter
