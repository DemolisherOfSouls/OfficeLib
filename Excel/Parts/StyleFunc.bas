...
Public Sub AddTitle()
  Dim TCell As Range: TCell = StyleCode.Offset(-1, 0)
...
Public Sub UpdateStyles()
  ...
  For Counter = 1 To ActiveWorkbook.Styles.Count
    ...
    Dim Item As Style: Set Item = ActiveWorkbook.Styles(Counter)
    
    With Item
      If EndsWith(.Name, "Title") Then
        Let .Font.Size = Range("TitleFontSize_Override").Value
      ElseIf EndsWith(.Name, "Hd") Or EndsWith(.Name, "HdKey") Then
        Let .Font.Size = Range("HeaderFontSize_Override").Value
      ElseIf EndsWith(.Name, "Cell") Or EndsWith(.Name, "Box") Or EndsWith(.Name, "Key") Or _
             EndsWith(.Name, "Val") Or EndsWith(.Name, "Date") Then
       Let .Font.Size = Range("CellFontSize_Override").Value
      End If
      ...
      Let .IncludeFont = Range("SetsFont_Override").Value
      Let .IncludeNumber = Range("SetsFormat_Override").Value
    End With
